<script type="text/javascript">
	...
	function destroyEmail() {
		...
		$.ajax({
			...
			type:  'post',
			beforeSend: function () {
				$("#emails_messages").html('<div class="spinner-border text-info"> </div> {{ trans("forms.please_wait") }}');
			},
			success:  function (response) {

				$('#email option[value="'+email+'"]').remove();

				$("#emails_messages").html('<div class="alert alert-success fade show"> <button type="button" class="close" data-dismiss="alert">&times;</button> <strong> {{ trans("forms.well_done") }}! </strong> {{ trans("emails.success_destroy_email") }} </div>');
			}
		}).fail( function() {
    		$("#emails_messages").html('<div class="alert alert-warning fade show"> <button type="button" class="close" data-dismiss="alert">&times;</button> <strong> {{ trans("forms.warning") }}! </strong> {{ trans("emails.warning_destroy_email") }} </div>');
		});
