...
<form method="post" action="{{ route('administrators/prescribers/store') }}">
	...
	<div class="input-group mt-2 col-md-9 input-form">
		...
		<input type="number" class="form-control" name="provincial_enrollment" min="0" value="{{ old('provincial_enrollment') }}">

		<div class="input-group-prepend">
			<span class="input-group-text"> {{ trans('prescribers.national_enrollment') }} </span>
		</div>
		<input type="number" class="form-control" name="national_enrollment" min="0" value="{{ old('national_enrollment') }}">
	</div>
