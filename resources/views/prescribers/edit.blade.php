...
<form method="post" action="{{ route('prescribers/update', [$prescriber['id']]) }}">
	...
	<div class="input-group mb-6 col-md-9 input-form" style="margin-top: 1%">
		...
		<input type="text" class="form-control" name="last_name" value="{{ $prescriber['last_name'] }}" required>

		<div class="input-group-prepend">
			<span class="input-group-text"> {{ trans('prescribers.name') }} </span>
		</div>

		<input type="text" class="form-control" name="name" value="{{ $prescriber['name'] }}" required>
	</div>
