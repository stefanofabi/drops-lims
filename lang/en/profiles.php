<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Profile language lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during profile views for various messages that we must show the user.
    | You are free to modify these language lines according to your application's requirements.
    |
    */

    'my_profile' => 'My profile',
    'change_password' => 'Change password',
    'profile_index_message' => 'These are the most important data of your account. Here we will show all the detailed information of your account and how others will see you',
    'name' => 'Name',
    'last_name' => 'Last name',
    'email' => 'Email',
    'account_data' => 'Account data',
    'change_signature' => 'Change signature',
    'name_help' => 'This name is the one with which you will show yourself to other people',
    'email_help' => 'This email will be used to log in and for people to contact you',
    'last_name_help' => 'This last name is the one with which you will show yourself to other people',

];
